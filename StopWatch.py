...
window.config(bg=ORANGE, padx=50, pady=50)

# Canvas setup for the stopwatch background image
canvas = Canvas(width=150, height=150, bg=ORANGE, highlightthickness=0)
watch_image = PhotoImage(file="timer.png")  # Image for the timer
canvas.create_image(75, 75, image=watch_image)
canvas.grid(column=1, row=1)
# Timer text in the center of the canvas
timer = canvas.create_text(80, 83, text="00:00", font=FONT, fill=GRAY)

# Check mark label, appears after countdown ends
check_label = Label(text="", font=FONT, fg=RED, bg=ORANGE)
check_label.grid(column=1, row=3)

# Start button with start_timer function attached
start_button = Button(text="START", bg=ORANGE, fg=RED, font=FONT, command=start_timer)
start_button.grid(column=0, row=2)

# Reset button with reset_timer function attached
reset_button = Button(text="RESET", bg=ORANGE, fg=RED, font=FONT, command=reset_timer)
reset_button.grid(column=2, row=2)

# Timer label at the top of the UI
StopWatch_label = Label(text="Timer", fg=RED, bg=ORANGE, font=FONT)
